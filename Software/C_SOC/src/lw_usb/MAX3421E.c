...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	sendbuffer[0] = reg + 2;
	sendbuffer[1] = val;
	code = XSpi_Transfer(&SpiInstance, sendbuffer, NULL, 2);
	if (code != 0) {
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	code = XSpi_Transfer(&SpiInstance, sendbuffer, NULL, nbytes+1);
	if (code != 0) {
		 xil_printf("Error in MAXbytes_rd writing reg+2\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance,0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int code = XSpi_Transfer(&SpiInstance, sendbuffer, receivebuffer, 2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	code = XSpi_Transfer(&SpiInstance, sendbuffer, receivebuffer, nbytes+1);
	for (int i = 0; i < nbytes; i++) {
